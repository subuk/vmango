...
func New(ctx *web.Context) *mux.Router {
	...
	router.Handle("/machines/{hypervisor:[^/]+}/{name:[^/]+}/", web.NewHandler(ctx, handlers.MachineDetail)).Name("machine-detail")
	router.Handle("/machines/{hypervisor:[^/]+}/{name:[^/]+}/{action:(?:start|stop|reboot)}/", web.NewHandler(ctx, handlers.MachineStateChange)).Name("machine-changestate")
	router.Handle("/machines/{hypervisor:[^/]+}/{name:[^/]+}/delete/", web.NewHandler(ctx, handlers.MachineDelete)).Name("machine-delete")
